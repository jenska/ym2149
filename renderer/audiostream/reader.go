...
// MonoSource is the minimum interface a backend-neutral audio reader needs.
type MonoSource interface {
	DrainMonoF32([]float32) int
	OutputSampleRate() int
}

// Reader converts mono float32 samples into stereo float32 PCM bytes.
type Reader struct {
	source     MonoSource
	monoBuffer []float32
	pending    [8]byte
...
// NewReader creates a stereo PCM reader around a mono source.
...
func (r *Reader) OutputSampleRate() int {
	return r.source.OutputSampleRate()
}

// Read implements io.Reader and emits little-endian stereo float32 PCM.
...
// Underruns reports how many silent frames were injected because the source had no PCM ready.
